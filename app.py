...
def calcular_fila(entrada, salida, estado, almuerzo_min):
    ...
    if estado != "Trabajó":
        return 0, 0, 0, 0
    if entrada in (None, "") or salida in (None, "") or pd.isna(entrada) or pd.isna(salida):
        return 0, 0, 0, 0
    ...
    with st.expander(f"📅 Horario semanal — {nombre or 'Sin nombre'}", expanded=False):
        ...
        for _, row in editado.iterrows():
            ...
            turno, almuerzo, trabajadas, extra = calcular_fila(
                row["Entrada"], row["Salida"], row["Estado"], almuerzo_min
            )
            calc_rows.append({
                "Día": row["Día"],
                "Fecha": row["Fecha"],
                "Entrada": row["Entrada"].strftime("%H:%M") if row["Entrada"] else "-",
                "Salida": row["Salida"].strftime("%H:%M") if row["Salida"] else "-",
                "Estado": row["Estado"] or "-",
                "Turno": fmt_hm(turno),
